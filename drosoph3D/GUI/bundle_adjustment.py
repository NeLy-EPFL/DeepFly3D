...
prior_translation = [0, 0, 94]
prior_rotation = [0, None, 0]

def fun(params, cam_list, n_cameras, n_points, camera_indices, point_indices, points_2d, residual_mask=None):
    """Compute residuals.
...
def bundle_adjustment_sparsity(n_cameras, n_points, camera_indices, point_indices):
    ...
    m = camera_indices.size * 2 # + n_cameras * 3 + n_cameras * 3 + 3  # residuals, reprojection error for each dimension and translation error and rotation error and align error
    # notice that camera_incides.size gives the number of observations

    n = n_cameras * n_camera_params + n_points * 3  # all the parameters, 13 camera parameters and x,y,z values for n_points
    A = lil_matrix((m, n), dtype=int)  # sparse matrix

    i = np.arange(camera_indices.size)
    for s in range(n_camera_params):  # assign camera parameters to points residuals (reprojection error)
    ...
    '''
    range_i = np.arange(camera_indices.size * 2, camera_indices.size * 2 + n_cameras * 3)  # translation residual
    range_j = np.ravel([np.arange(3, 6) + (k * n_camera_params) for k in range(n_cameras)])
    for i, j in zip(range_i, range_j):
        A[i, j] = 1
    
    range_i = np.arange(camera_indices.size * 2 + n_cameras * 3,
                        camera_indices.size * 2 + n_cameras * 3 + n_cameras * 3)  # rotation residual
    range_j = np.ravel([np.arange(0, 3) + (k * n_camera_params) for k in range(n_cameras)])  # camera rotation
    for i, j in zip(range_i, range_j):
        A[i, j] = 1

    # add 1 to camera 2 and 3 rotations w.r.t. align residual
    range_i = np.arange(camera_indices.size * 2 + n_cameras * 3 + n_cameras * 3,
                        camera_indices.size * 2 + n_cameras * 3 + n_cameras * 3 + 3)
    range_j = np.ravel(
        [np.arange(0, 3) + (k * n_camera_params) for k in [2, 3]])  # camera rotation of second and third cameras
    for i, j in zip(range_i, range_j):
        A[i, j] = 1
    '''
