...
def image_overlay_pose(inp, pts, pts_max_value, joint_idx=None, joint_draw=None):
    ...
    for bone in skeleton.bones:
        if bone[0] < pts.shape[0] and bone[1] < pts.shape[0] and bone[0] in joint_draw and bone[1] in joint_draw:  # \
            ...
            cv2.line(inp, (pts[bone[0]][0], pts[bone[0]][1]), (pts[bone[1]][0], pts[bone[1]][1]), color=color,
                     thickness=1)
...
def drosophila_image_overlay(inputs, target, hm_res, batch_size, train_joints, img_id=None):
    gt = get_preds(target)
    ...
    for img_idx in range(batch_size):
        img_overlay_list.append(
            image_overlay_pose(inputs[img_idx], gt[img_idx], joint_idx=[],
                               joint_draw=train_joints, pts_max_value=hm_res))
    ...
    img_overlay_hm_stack = (array.reshape(nrows, ncols, height, width, intensity)
                            .swapaxes(1, 2)
                            .reshape(height * nrows, width * ncols, intensity))

    img_overlay_hm_stack = resize(img_overlay_hm_stack,
    ...
    if img_id is not None:
        img = cv2.putText(img, "{}".format(img_id), (img.shape[1] - 150, 50), cv2.FONT_HERSHEY_PLAIN, 4,
