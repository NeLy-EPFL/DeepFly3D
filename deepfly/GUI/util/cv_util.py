...
def triangulate(P1, P2, points1, points2):
    points3d = cv2.triangulatePoints(
        P1, P2, points1.T.astype(float), points2.T.astype(float)
    )
...
def hom_to_eucl(points3d):
    points3d = points3d * (1.0 / points3d[:, 3][:, np.newaxis])
    return points3d[:, :3]
...
def project_points(
    ...
    points2d, jacobian = cv2.projectPoints(
        objectPoints=points3d.T, rvec=R, tvec=t, cameraMatrix=intr, distCoeffs=distort
    )
