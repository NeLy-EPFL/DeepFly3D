...
from .optimLM import project_on_last, energy_drosoph
...
class LegBP:
    def __init__(self, camera_network, img_id, j_id_list, bone_param, num_peak=10, prior=None, upper_bound=200,
        ...
        self.alpha_reproj = 30  # 30
        self.alpha_heatmap = 600  # 600
        self.alpha_bone = 100  # 10
    ...
    def generate_proposals(self, num_peak, prior=None):
        for j in self.jointbp:
            cam_id_list_seeing_joint = [cam_id for cam_id in self.cam_id_list if
                                        skeleton.camera_see_joint(cam_id, j.j_id)]
            p2d_list = []
            # find 2d proposals for a given joint for each camera, by taking local maximums
            for cam in self.camera_network:
                min_distance = 1
                threshold_abs = 0.0
                threshold_rel = 0.95 if skeleton.is_body_coxa(j.j_id) or skeleton.is_coxa_femur(
                    j.j_id) or skeleton.is_antenna(j.j_id) else 0.1

                p2d_list.append(
                    Camera.hm_to_pred(np.squeeze(cam.get_heatmap(self.img_id, j.j_id)), num_pred=num_peak,
            ...
            if prior is not None:
                for cam_id, joint_id, pts in prior:
                    if joint_id == j.j_id:
                        ...
                        p2d_list[cam_index] = [pts]  # we remove all the other heatmap proposals
            ...
            for p2d_prop in list(itertools.product(*p2d_list)):
                ...
                for cam, p2d in zip(self.camera_network.cam_list, p2d_prop):
                    if skeleton.camera_see_joint(cam.cam_id, j.j_id):
                        p2d_list_iter.append(p2d)
                        cam_list_iter.append(cam)
                p2d_list_iter = np.array(p2d_list_iter).reshape(-1, 2)
                p3d, err_proj, prob_hm, _ = energy_drosoph(cam_list_iter, self.img_id, j.j_id, p2d_list_iter, None,
                                                           None)
                prob_hm += len([cam_id for cam_id in cams_with_prior if skeleton.camera_see_joint(cam_id, j.j_id)])
            ...
            for camid_x, camid_y, camid_z in itertools.permutations(cam_id_list_seeing_joint, 3):
                cam_index_x, cam_index_y, cam_index_z = self.cam_id_list.index(camid_x), self.cam_id_list.index(
                    camid_y), self.cam_id_list.index(camid_z)
                cam_x, cam_y, cam_z = self.cam_list[cam_index_x], self.cam_list[cam_index_y], self.cam_list[cam_index_z]
                # iterate over all combinations of 2d proposals
                for p2d_x, p2d_y, p2d_z in list(itertools.product(*[p2d_list[cam_index_x], p2d_list[cam_index_y],
                    ...
                    for cam_index_project in [cam_index for cam_index in range(len(self.cam_id_list)) if
                                              cam_index not in [cam_index_x, cam_index_y, cam_index_z]]:
                        p2d_list_iter[cam_index_project] = project_on_last(
                            [cam_x, cam_y, cam_z, self.cam_list[cam_index_project]],
                            np.array([p2d_x, p2d_y, p2d_z]).reshape(-1,
                                                                    2) * self.image_res) / self.image_res
                    p2d_list_iter = np.array(p2d_list_iter).reshape(-1, 2)
                    p3d, err_proj, prob_hm, _ = energy_drosoph(self.camera_network.cam_list, self.img_id, j.j_id,
                                                               p2d_list_iter, None, None)
                    prob_hm += len([cam_id for cam_id in cams_with_prior if
                                    (skeleton.camera_see_joint(cam_id, j.j_id) and cam_id in [camid_x, camid_y,
                                                                                              camid_z])])
    ...
    def propagate(self):  # start from the leaf, calculate belief for each candid
        for c in self.jointbp[-1].candid_list:  # the only leaf
            c.belief = 1
        for idx in range(len(self.j_id_list) - 1, 0, -1):  # finish at the root (at 0) (excluding)
            for cand_p in self.jointbp[idx - 1].candid_list:
                j_c = self.jointbp[idx]
                cand_p.belief = min([self.B_j(cand_c, cand_p) for cand_c in j_c.candid_list])

    def B_j(self, cand_c, cand_p):  # function of parent node, cost function
        return self.m_j(cand_c) + self.alpha_bone * (
                1 - self.d_ij(cand_c.p3d, cand_p.p3d, self.bone_param[cand_p.j_id], cand_p.j_id)) + cand_c.belief
    ...
    def d_ij(self, p3d_c, p3d_p, param, joint_id):
        ...
        if joint_id % 5 != 4:
            ...
            if dist > mu + sig:
                return np.exp(-np.power(dist - mu, 2.) / (2 * np.power(sig / 5, 2.)))
            else:
                return np.exp(-np.power(dist - mu, 2.) / (2 * np.power(sig * 3, 2.)))

    def m_j(self, cand):
        return self.alpha_reproj * cand.err_proj + self.alpha_heatmap * (1 - cand.prob_hm)

    def solve(self):  # start from the root, set the joint with the largest belief
        root_j = self.jointbp[0]
        belief_list = [self.m_j(candid) + candid.belief for candid in root_j.candid_list]
        ...
        for idx in range(1, len(self.j_id_list)):  # exclude root, already solved
            self.jointbp[idx].argmin = np.argmin([self.B_j(cand_c, cand_p) for cand_c in
                                                  self.jointbp[idx].candid_list])
