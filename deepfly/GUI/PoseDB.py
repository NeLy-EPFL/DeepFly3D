...
class PoseDB:
    def __init__(self, folder, meta=None):
        ...
        if len(self.db_path_list) != 0:
            ...
            if 'train' not in self.db:
                self.db["train"] = {i: dict() for i in range(skeleton.num_cameras)}
            if 'modified' not in self.db:
                self.db["modified"] = {i: dict() for i in range(skeleton.num_cameras)}
        else:
            self.db_path = os.path.join(self.folder, "pose_corr_{}.pkl".format(self.folder.replace("/", "-")))
            self.db = {i: dict() for i in range(skeleton.num_cameras)}
    ...
    def write(self, pts, cam_id, img_id, train=False, modified_joints=None):
        assert (pts.shape[0] == skeleton.num_joints and pts.shape[1] == 2)
